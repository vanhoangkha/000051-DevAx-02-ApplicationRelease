...
Tiếp theo, bạn sẽ cần phải đóng gói artefact triển khai của mình (có thể là trang TravelBuddy mà chúng tôi đang sử dụng hoặc bạn có thể tạo ứng dụng web java của riêng mình) và tải tệp WAR đó lên S3. Bạn đã học cách tải tệp lên bộ chứa S3 theo chương trình trong bài thực hành đầu tiên của khóa học này. 
...
```
Ta có toàn bộ đoạn code tạo Application và Enviroment như sau:
```java
public void CreateApp(){
	CreateApplicationRequest request = new CreateApplicationRequest()
			.withApplicationName("my-app")
...
```
Và toàn bộ các thư viện cần import:
```java
import com.amazonaws.services.elasticbeanstalk.model.CreateApplicationRequest;
import com.amazonaws.services.elasticbeanstalk.model.CreateApplicationResult;
import com.amazonaws.services.elasticbeanstalk.model.CreateApplicationVersionRequest;
