...
4. Ấn **Download remote desktop file**

![ConnecttoEC2Instance](/images/5/5.png?width=90pc)

5. Chọn keypair **KPforDevAxInstances** để decrypt mật khẩu của máy ảo Windows.
- Ấn **Decrypt password**.
...
7. Trong máy ảo Windows, mở tập tin **c:\temp\host-agent-install-log**
8. Kiểm tra và đảm bảo rằng không có dòng nào có báo lỗi, ví dụ như *Failed to connect to server* vì đây là một lỗi xảy ra trong quá trình cài đặt.
9. Nếu có lỗi xảy ra trong quá trình cài đặt, bạn có thể thử tự chạy lại bằng các câu lệnh sau:
```bash
c:\temp\codedeploy-agent.msi /l*v c:\temp\host-agent-install-log.txt
```
10. Sau khi chạy lại cài đặt, kiểm tra lại tập tin **c:\temp\host-agent-install-log**. Nếu không có lỗi nào được hiển thị, quá trình tạo mới CodeDeploy có khả năng thành công. Bạn có thể tạo mới bằng lệnh AWS CLI:
```bash
...
Giả sử CodeDeploy agent được cài đặt và chạy thành công, và bạn muốn xác nhận việc triển khai dịch vụ Heartbeat thành công, bạn có thể thực hiện các bước sau đây:

1. Sử dụng Service Control Manager để tìm **AWS Heartbeat Demo Service**. Nếu dịch vụ này hiển thị *Running* trong SCM, dịch vụ này đã cài đặt thành công.
2. Mở thư mục **c:\Logs**. Tập tin **HeartBeatService** trong thư mục sẽ chứa ouput như sau nếu nó khởi chạy thành công:
```
